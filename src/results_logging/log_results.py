...
from config import DEFAULT_MODEL_NAME, EMB_MODEL_NAME, RESULT_LOG_DIR

logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')

def save_results_to_json(user_query : str = None, retrieved_documents : List[Document] = None, prompt_template : str = None, generated_answer : str = None, embedding_model_name : str = None, LLM_name : str = None, filename=f"{RESULT_LOG_DIR}/conversation_logs.json"):
   """
...
def log_chain_results(result, prompt):
   ...
   LLM_name = DEFAULT_MODEL_NAME

   # Call to save the logs
   logging.info(f"saving outputs in {log_file_path}")
   save_results_to_json(user_query, retrieved_documents, prompt_template, generated_answer, 
            embedding_model_name, LLM_name, filename=f"{RESULT_LOG_DIR}/conversation_logs.json")
