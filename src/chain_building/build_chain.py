...
def build_chain(
    ...
    if reranker is None:
        ...
        retrieval_agent = RunnableParallel(
            ...
            lambda r: compress_documents_lambda(
                documents=r["documents"], query=r["query"], k=10
            )
        )
        ...
        compressor = CrossEncoderReranker(
            model=HuggingFaceCrossEncoder(model_name=RERANKER_CROSS_ENCODER), top_n=10
        )
        ...
        reranker_3 = ContextualCompressionRetriever(
            base_compressor=RAGPretrainedModel.from_pretrained(
                RERANKER_COLBERT
            ).as_langchain_document_compressor(k=10),
            base_retriever=retriever,
        )
        ...
        raise ValueError(
            "This reranking method is not handled by the ChatBot or does not exist"
        )
    ...
    if bool_log:
        return rag_chain_with_source | RunnableLambda(log_chain_results).bind(
...
def build_chain_retriever(retriever, bool_log: bool = False, reranker=None):
    ...
    if bool_log:
        return retriever_chain | RunnableLambda(log_chain_results).bind(
            prompt=None, reranker=reranker
        )
    else:
