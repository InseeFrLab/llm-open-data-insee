...
def compress_BM25_lambda(
    documents: Sequence[Document], query: str, k: int = 5, **kwargs: Dict[str, Any]) -> Sequence[Document]:
...
def compress_metadata_lambda(
    documents: Sequence[LangchainDocument], query: str, config: Dict) -> Sequence[LangchainDocument]:
...
def build_chain_reranker_test(config=RetrievalConfiguration):
    ...
    if reranker_type in [None, "BM25", "Cross-encoder", "ColBERT", "Metadata"]:
        ...
        retrieval_agent = results | RunnableLambda(func=lambda d: aggregate_ensemble_reranker(d)) | RunnableLambda(func=lambda d: weighted_reciprocal_rank(doc_lists=d, weights=weights))
