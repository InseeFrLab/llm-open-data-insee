...
def extract_chunks(row) -> List:
    ...
    for chunk_tag in CHUNK_TAGS:
        while True:
            ...
            if chunk.text.strip() != "":
                chunks.append(
                    {"xml": chunk.extract().decode(), "tag": "other", "id": identifier}
                )
    ...
    if "xml_content" in table.columns:
        for i, row in tqdm(table.iterrows()):
            try:
                ...
                if (
                    len(paras) == 0
                ):  # handle badly indented documents by using an xml parser
                    tag = "paragraphe"
                ...
                if (
                    len(para) > 0
                ):  # filtering to only keep documents with textual informations.
