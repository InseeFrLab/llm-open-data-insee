...
async def on_chat_start():
    ...
    if res and res.get("value") == "log":
        await cl.Message(
            content="Vous avez choisi de partager vos intéractions."
        ).send()
        bool_log = True
    if res and res.get("value") == "no_log":
        bool_log = False
        await cl.Message(
            content="Vous avez choisi de garder vos intéractions avec le ChatBot privées."
        ).send()
    ...
    res = await cl.AskActionMessage(
        ...
        actions=[
            ...
            cl.Action(
                name="BM25",
                value="BM25",
                description="Choisir un reranker lexical BM25",
            ),
            cl.Action(
                name="Cross-encoder",
                value="Cross-encoder",
            ...
            cl.Action(
                name="Ensemble",
                value="Ensemble",
                description="Choisir un reranker Hybride",
            ),
        ],
        timeout=15,
    ).send()
    ...
    if chat_profile == "RAG":
        chain = build_chain(
            retriever, prompt, llm, bool_log=bool_log, reranker=reranker
        )
    else:
...
def end():
    ...
    try:
        ...
        print(result.stdout)
        print(
            "Someone ended this chat: conversation logging file(s) have been copied successfully."
        )
