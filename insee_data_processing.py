...
FILES = ["applishare_extract", "solr_extract", "rmes_extract"]
...
joined_table = tables["applishare_extract"].merge(tables["solr_extract"], how="left", on="id")
...
empty_url_x, filled_url_x = (joined_table[joined_table["url_x"] == ""], joined_table[joined_table["url_x"] != ""])
...
rmes_additional_pages = rmes_additional_pages.rename(columns={"title": "titre", "xml_content": "content"})

rmes_data_complete = pd.concat([filled_url_x, rmes_additional_pages])
...
filled_url_x = filled_url_x.drop(columns="xml_content")


# Convertir en DataFrame
filled_url_x = pd.merge(filled_url_x, pd.DataFrame(results), on="id")

filled_url_x = filled_url_x.rename(columns={"content": "xml_content"})


# ADD SOLR DATA ------------------------------------------

# 2. Split based on whether url_y is filled or not
url_y_filled, url_y_missing = (empty_url_x[empty_url_x["url_y"] != ""], empty_url_x[empty_url_x["url_y"] == ""])

# FILL url ------------------------------

url_y_missing = complete_url_builder(url_y_missing)

joined_table = pd.concat([url_y_missing, url_y_filled, filled_url_x])
